...
export default function MyPostScreen() {
  ...
  const [category, setCategory] = useState(''); const [title, setTitle] = useState(''); const [description, setDescription] = useState('');
  const [budgetType, setBudgetType] = useState('Fixed Price'); const [budget, setBudget] = useState(''); const [contact, setContact] = useState('Chat');
  ...
  const [region, setRegion] = useState(''); const [city, setCity] = useState(''); const [area, setArea] = useState(''); const [address, setAddress] = useState('');
  ...
  const validateDetails = () => {
    if (!title.trim() || !category || description.trim().length < 10 || !region || !city || !area.trim()) {
      Alert.alert('Complete required details', 'Add a title, category, description (10+ characters), and location before continuing.'); return false;
    }
    return true;
  };
  const submit = async () => {
    ...
    try {
      await createPostApi({
        ...
        serviceType: type === 'service' ? category : undefined,
  ...
  return <View style={[styles.root, { paddingTop: insets.top }]}>
    ...
    <ScrollView contentContainerStyle={[styles.content, { paddingBottom: 115 + insets.bottom }]} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
      {step === 1 && <><Text style={styles.heading}>What are you looking for?</Text><Text style={styles.subheading}>Choose the type of request you want to post.</Text><View style={styles.typeRow}>
        <TouchableOpacity style={[styles.typeCard, isProduct && styles.typeCardSelected]} onPress={() => { setType('product'); setCategory(''); }}><View style={[styles.typeIcon, { backgroundColor: '#eaf5ef' }]}><Ionicons name="cube" size={31} color={GREEN} /></View><Text style={styles.typeTitle}>Product</Text><Text style={styles.typeDesc}>I want to buy or find{`\n`}a product</Text>{isProduct && <Ionicons style={styles.cornerCheck} name="checkmark-circle" size={22} color={GREEN} />}</TouchableOpacity>
        <TouchableOpacity style={[styles.typeCard, type === 'service' && styles.typeCardSelected]} onPress={() => { setType('service'); setCategory(''); setImages([]); }}><View style={[styles.typeIcon, { backgroundColor: '#fff3e5' }]}><Ionicons name="construct" size={31} color={AMBER} /></View><Text style={styles.typeTitle}>Service</Text><Text style={styles.typeDesc}>I need someone to{`\n`}do a service</Text>{type === 'service' && <Ionicons style={styles.cornerCheck} name="checkmark-circle" size={22} color={GREEN} />}</TouchableOpacity>
      </View></>}
      {step === 2 && <>
        <View style={styles.card}><Text style={styles.cardTitle}>{isProduct ? 'Product Details' : 'Service Details'}</Text><Text style={styles.cardHint}>Tell sellers exactly what you need.</Text><TextField label={isProduct ? 'Product Title' : 'Service Title'} required value={title} onChange={setTitle} placeholder={isProduct ? 'e.g. Samsung Galaxy S24, 256GB, Black' : 'e.g. Need a professional plumber'} /><Select label={isProduct ? 'Product Category' : 'Service Category'} required value={category} placeholder="Select a category" options={isProduct ? PRODUCT_CATEGORIES : SERVICE_CATEGORIES} onChange={setCategory} /><TextField label="Description" required value={description} onChange={setDescription} placeholder={isProduct ? 'Describe the product you are looking for in detail...' : 'Describe the service you need in detail...'} multiline /></View>
        ...
        <Budget budgetType={budgetType} setBudgetType={setBudgetType} budget={budget} setBudget={setBudget} /><Location region={region} setRegion={setRegion} city={city} setCity={setCity} area={area} setArea={setArea} address={address} setAddress={setAddress} />
      </>}
      {step === 3 && <>
        {isProduct ? (
        ...
        <View style={styles.card}><Text style={styles.cardTitle}>Review your request</Text><Summary label="Type" value={isProduct ? 'Product' : 'Service'} /><Summary label="Title" value={title} /><Summary label="Category" value={category} /><Summary label="Location" value={`${area}, ${city}`} /><Summary label="Budget" value={budget ? `GHS ${budget} · ${budgetType}` : budgetType} /></View>
      </>}
    </ScrollView>
...
function Budget({ budgetType, setBudgetType, budget, setBudget }: { budgetType: string; setBudgetType: (v: string) => void; budget: string; setBudget: (v: string) => void }) { return <View style={styles.card}><Text style={styles.cardTitle}>Budget</Text><Field label="Budget Type" required><View style={styles.segmented}>{['Fixed Price', 'Price Range', 'Negotiable'].map((value) => <TouchableOpacity key={value} style={[styles.segment, budgetType === value && styles.segmentSelected]} onPress={() => setBudgetType(value)}><Text style={styles.segmentText}>{value}</Text></TouchableOpacity>)}</View></Field><TextField label="Budget Amount" required={budgetType !== 'Negotiable'} value={budget} onChange={setBudget} placeholder="GHS   Enter amount" keyboardType="numeric" /></View>; }
function Location({ region, setRegion, city, setCity, area, setArea, address, setAddress }: any) { return <View style={styles.card}><Text style={styles.cardTitle}>Location</Text><View style={styles.twoCols}><Select label="Region" required value={region} placeholder="Select region" options={REGIONS} onChange={setRegion} /><Select label="City / Town" required value={city} placeholder="Select city" options={CITIES} onChange={setCity} /></View><TextField label="Area / Neighborhood" required value={area} onChange={setArea} placeholder="Enter your area" /><TextField label="Exact Address (Optional)" value={address} onChange={setAddress} placeholder="e.g. House number, street name" /></View>; }
